<script>
  ...
  import ActiveGrid from '../simpsons/ActiveGrid.svelte'
  ...
  function makeColumns(
    ...
    let cells = []
    let majScale = scaleBand().domain([0, 1]).range([0, width]).padding(0.25)
  ...
  $: {
    ...
    majGrids.labels = [
      { x: $width / 2, y: $padding.bottom / 2, stats: getGroupStats($data) },
    ]
  ...
  {#if context}
    {#each context.labels as label}
      <div
        ...
        style={`left: ${label.x}px; bottom: -${label.y}px`}
      >
        <Eq data={label.stats} />
      </div>
    {/each}
